...
.metaAnalysisGetOutputContainer <- function(jaspResults) {
  if (!is.null(jaspResults[["modelContainer"]])) {
    ...
    modelContainer$dependOn(c("effectSize", "effectSizeStandardError", "method", "studyLabels", "covariates", "regressionCoefficientEstimateTest",
...
.metaAnalysisCheckErrors <- function(dataset, options){
  ...
  .hasErrors(dataset              = dataset,
             ...
             negativeValues.target= options$effectSizeStandardError,
             exitAnalysisIfErrors = TRUE)
}
